...
from upscal_class import Up_Scale
...
class Image2form():
    ...
    # *************** TKH用にinput_imageをフォーミングする。out_formで出力をpilかcvに指定、mask：True= マスク生成,del_flegで背景削除の有無を指定する。
    #    例）Talking-Head用アライメント画像作成
    #        input_image =image_data_form(input_image ,"pil",False)

    def image_data_form(self,
                input_image,
                out_form="pil",
                h=512,
                w=512,
        ...
        imge, dnum, predict_bbox, pre_dict_label_index, scores =self.AF.face_det_head(cv_w_img,1.68,0.5, confidence_level)#face-head検出   ratio=1.68, shift=0.5, confidence_level=0.5
        ...
        try:
            ...
            resize_facter=head/int(box[0]-box[2])
        ...
        pil_out_image = Image.new("RGBA", (w, h), (0, 0, 0, 0))
        # ペーストする位置を指定
        p_top = top_space-top #バウンディングboxの位置が64pixよりも大きければ差がペースト位置、小さければ差がマイナスになっているがpilではOK
        p_left = int(w/2)-int(head/2)-left
        #p_left =192-left
...
#    def image_upscale(self ,input_image ,shape ,scale, out_form="pil"):
